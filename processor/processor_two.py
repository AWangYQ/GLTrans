...
def do_train(cfg,
    ...
    if device:
        model_refine.to(local_rank)
        model_original.to(local_rank)
        if torch.cuda.device_count() > 1 and cfg.MODEL.DIST_TRAIN:
            print('Using {} GPUs for training'.format(torch.cuda.device_count()))
            model = torch.nn.parallel.DistributedDataParallel(model_refine, device_ids=[local_rank], find_unused_parameters=True)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, vid, target_cam, target_view) in enumerate(train_loader):
            ...
            with torch.no_grad():
                local_feats, global_feats, cls_token = model_original(img, target, cam_label=target_cam, view_label=target_view)
            with amp.autocast(enabled=True):
                score, feat = model_refine(local_feats, global_feats, cls_token)
        ...
        if epoch % eval_period == 0:
            ...
            for n_iter, (img, pid, camid, camids, target_view, imgpath) in enumerate(val_loader):
                with torch.no_grad():
                    ...
                    local_feats, global_feats, cls_token = model_original(img, cam_label=camids, view_label=target_view)
                    feat = model_refine(local_feats, global_feats, cls_token)
...
def do_inference(cfg,
    ...
    for n_iter, (img, pid, camid, camids, target_view, imgpath) in enumerate(val_loader):
        with torch.no_grad():
            ...
            camids = camids.cuda()
            target_view = target_view.cuda()
            local_tokens, global_tokens, cls = model_original(img, cam_label=camids, view_label=target_view)
            feat = model_refine(local_tokens, global_tokens, cls)
            evaluator.update((feat, pid, camid, imgpath))
            img_path_list.extend(imgpath)
